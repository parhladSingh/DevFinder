...
//                   height={30}
//                   alt="Bell Icon"
//                   className="ml-20 sm:w-8 sm:h-8 md:w-10 md:h-10 lg:w-12 lg:h-12"
//                 />
...
//                       onClick={() => {
//                         console.log(
//                           `Reject button clicked for: ${notification.roomId}`
//                         );
//                         socket.emit("opponent_rejected", {
//                           roomId: notification.roomId,
//                           email: notification.opponentUserData.email, // Send email if needed
//                         });
//                         setIsNotificationBoxOpen(false);
//                       }}
//                     >
...
const Navbar = () => {
  ...
  const handleJoinCall = (notification: Notification) => {
    ...
    // Clear notifications after joining
    setAllNotifications([]);
  };

  // Function to handle rejecting a call
  const handleRejectCall = (notification: Notification) => {
    socket.emit("opponent_rejected", {
      roomId: notification.roomId,
      email: notification.opponentUserData.email,
    });
    ...
    // Clear notifications after rejecting
    setAllNotifications([]);
  };
            ...
            <div className="relative">
              ...
              >
                <Image
                  ...
                  height={30}
                  alt="Bell Icon"
                  className="ml-16 mr-10 sm:w-8 sm:h-8 sm:mr-10 md:w-10 md:h-10 lg:w-12 lg:h-12"
                />
                <sup className="font-bold text-sm sm:text-sm md:text-lg lg:text-xl text-[#FFD700] hover:text-[#FF6347]">
                  {allNotifications.length > 0 ? allNotifications.length : ""}
                </sup>
                ...
                {allNotifications.map((notification, index) => (
                  ...
                  >
                    ...
                    <Button
                      variant="secondary"
                      className="px-1 sm:px-2 bg-purple-700 hover:bg-purple-500 font-bold  mx-1 sm:mx-1"
                      onClick={() => handleJoinCall(notification)} // Use the join handler
                    >
                      Join
                    </Button>
                    ...
                    <Button
                      variant="secondary"
                      className="px-1 sm:px-2 bg-purple-700 hover:bg-purple-500 font-bold mx-1 sm:mx-1"
                      onClick={() => handleRejectCall(notification)} // Use the reject handler
                    >
                      Reject
                    </Button>
